...
async function respondToAssessmentRequest(requestId, tutorId, action, message = '', itemCount = null) {
  ...
  const updateFields = itemCount
    ? `UPDATE assessment_requests SET status = ?, responded_at = SYSDATETIME(), tutor_message = ?, item_count = ?, updated_at = SYSDATETIME() WHERE id = ?`
    : `UPDATE assessment_requests SET status = ?, responded_at = SYSDATETIME(), tutor_message = ?, updated_at = SYSDATETIME() WHERE id = ?`;
