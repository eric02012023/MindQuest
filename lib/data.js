...
async function submitAssessment(assessmentId, studentId, payload) {
  return withTransaction(async (connection) => {
    ...
    const perModulePayload = JSON.stringify(perModuleScores);
    ...
    if (existing) {
      await connection.query(
        ...
        [score, questionRows.length, Number(percentage.toFixed(2)), level, answersPayload, perModulePayload, assessmentId, studentId]
      );
    } else {
      await connection.query(
        `INSERT INTO assessment_results (assessment_id, student_id, score, total_questions, percentage, level, answers_json, taken_at, per_module_scores_json)
         VALUES (?, ?, ?, ?, ?, ?, ?, DATEADD(hour, 8, GETUTCDATE()), ?)`,
        [assessmentId, studentId, score, questionRows.length, Number(percentage.toFixed(2)), level, answersPayload, perModulePayload]
      );
    }
...
async function getAllModulesAdmin() {
  return query(
    `SELECT m.*, s.name as subject_name, u.first_name, u.last_name 
     ...
     ORDER BY s.name ASC, m.level ASC`
  );
}
