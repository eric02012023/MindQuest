...
async function syncStudentSubjectAssignments(connection, studentId, subjectNames = [], branchId = null, adminId = null) {
  ...
  for (const subjectName of normalizedNames) {
    ...
    if (activeRow) {
      await connection.query(
        'UPDATE user_subject_assignments SET branch_id = ?, updated_at = SYSDATETIME() WHERE id = ?',
      ...
      for (const duplicate of existingRows.filter((row) => Number(row.id) !== Number(activeRow.id) && Number(row.is_archived || 0) === 0)) {
        await connection.query(
          'UPDATE user_subject_assignments SET is_archived = 1, updated_at = SYSDATETIME() WHERE id = ?',
          [duplicate.id]
        );
      }
      continue;
    ...
    if (existingRows.length) {
      ...
      await connection.query(
        `UPDATE user_subject_assignments
         SET is_archived = 0, tutor_id = NULL, branch_id = ?, accepted_by = ?, enrolled_at = COALESCE(enrolled_at, SYSDATETIME()), updated_at = SYSDATETIME()
         WHERE id = ?`,
    ...
    await connection.query(
      ...
      ) VALUES (?, NULL, ?, ?, SYSDATETIME(), NULL, ?, 0)`,
  ...
  for (const assignment of assignmentRows) {
    if (!allowedSubjectIds.has(Number(assignment.subject_id))) {
      await connection.query(
        'UPDATE user_subject_assignments SET tutor_id = NULL, is_archived = 1, updated_at = SYSDATETIME() WHERE id = ?',
        [assignment.id]
      );
    }
  }
...
async function cleanupStudentTutorAssignments(connection, studentId, studentSnapshot = null) {
  ...
  for (const row of rows) {
    ...
    if (!scopeMatch.isMatch) {
      await connection.query(
        'UPDATE user_subject_assignments SET tutor_id = NULL, updated_at = SYSDATETIME() WHERE id = ?',
        [row.id]
      );
    }
  }
...
async function markNotificationRead(id, scopeBranchId = null) {
  ...
  await query('UPDATE submissions SET read_at = SYSDATETIME() WHERE id = ?', [notification.submission_id]);
  return true;
}
...
async function archiveNotification(id, scopeBranchId = null) {
  ...
  await query('UPDATE submissions SET archived = 1, archived_at = SYSDATETIME() WHERE id = ?', [notification.submission_id]);
  return true;
}
...
async function acceptNotification(id, actor) {
  return withTransaction(async (connection) => {
    ...
    const [insertResult] = await connection.query(
      ...
      ) VALUES (?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSDATETIME(), 'approved', 0)`,
      [
        tempUserCode,
        row.submission_type,
    ...
    await connection.query('UPDATE submissions SET status = ?, accepted_at = SYSDATETIME(), read_at = SYSDATETIME() WHERE id = ?', ['accepted', row.id]);
...
async function getRecentSubmissions(scopeBranchId = null, limit = 8) {
  ...
  const rows = await query(
    `SELECT TOP ${Number(limit)} s.*, b.name AS branch_name, n.id AS notification_id, n.is_read, n.is_archived, n.moved_to_history
     ...
     WHERE s.created_at >= DATEADD(DAY, -3, SYSDATETIME()) ${scope.sql}
...
async function changeUserPassword(id, currentPassword, newPassword) {
  ...
  await query(
    'UPDATE users SET password_hash = ?, extra_json = ?, updated_at = SYSDATETIME() WHERE id = ?',
...
async function updateUser(id, payload) {
  ...
  return withTransaction(async (connection) => {
    ...
    if (user.role === 'student') {
      ...
      await connection.query('UPDATE billing SET updated_at = SYSDATETIME() WHERE student_id = ?', [id]);
    }
  ...
  return withTransaction(async (connection) => {
    ...
    if (!existingAssignmentRows.length) {
      ...
      await connection.query(
        ...
        ) VALUES (?, ?, ?, ?, SYSDATETIME(), ${inheritedTutor ? 'SYSDATETIME()' : 'NULL'}, ?, 0, ?)`,
    ...
    await connection.query(
      'UPDATE users SET subjects_json = ?, updated_at = SYSDATETIME() WHERE id = ?',
      [JSON.stringify([...currentSubjects]), row.student_id]
    );

    await connection.query(
      `UPDATE subject_enrollment_requests
       SET status = 'accepted', decided_by = ?, decided_at = SYSDATETIME(), updated_at = SYSDATETIME()
       WHERE id = ?`,
      [actor.id, id]
    );
    return true;
...
async function getTutorDashboardData(tutorId) {
  ...
  return { user, assignments, subjects, totalStudents: assignments.length };
}
...
async function updateBilling(studentId, payload, adminId) {
  ...
  await query(
    `UPDATE billing SET
      ...
      soa_posted_at = CASE WHEN ? = 1 THEN SYSDATETIME() ELSE soa_posted_at END,
...
async function markBillPaid(billingId, adminId) {
  ...
  await query(
    `UPDATE billing SET partial_payment = full_bill, for_settlement = 0.00, payment_status = 'paid', last_paid_at = SYSDATETIME(), posted_by = ? WHERE id = ?`,
    [adminId, billingId]
  );
  await query(
    'INSERT INTO payment_history (billing_id, student_id, amount, paid_at, recorded_by) VALUES (?, ?, ?, SYSDATETIME(), ?)',
...
async function postSoa(studentId, adminId) {
  ...
  await query('UPDATE billing SET soa_posted_at = SYSDATETIME(), posted_by = ? WHERE id = ?', [adminId, bill.id]);
  return true;
}
...
async function addSubject(name) {
  ...
  if (existing.length) {
    if (Number(existing[0].is_archived) === 1) {
      await query('UPDATE subjects SET name = ?, is_archived = 0, updated_at = SYSDATETIME() WHERE id = ?', [normalized, existing[0].id]);
      return existing[0].id;
    }
...
async function assignStudentsToTutor(subjectId, tutorId, studentIds, adminId) {
  ...
  for (const row of validRows) {
    await query(
      `UPDATE user_subject_assignments
       SET tutor_id = ?, assigned_at = SYSDATETIME(), accepted_by = ?, branch_id = ?, updated_at = SYSDATETIME()
       WHERE id = ?`,
...
async function getAdminSubjectResources(subjectId) {
  const rows = await query(
    `SELECT sr.*, u.first_name, u.middle_name, u.last_name
     ...
     WHERE sr.subject_id = ? AND sr.created_by_role = 'admin_template' AND ISNULL(sr.is_archived, 0) = 0
...
async function updateMessageBody(id, body) {
  await query(
    'UPDATE messages SET body = ?, edited_at = SYSDATETIME() WHERE id = ?',
    [String(body || '').trim(), id]
  );
}
...
async function unsendMessage(id) {
  await query(
    `UPDATE messages
     SET body = '', file_path = NULL, file_original_name = NULL, file_type = '', is_unsent = 1, edited_at = SYSDATETIME()
     WHERE id = ?`,
    [id]
  );
}
...
async function submitAssessment(assessmentId, studentId, payload) {
  return withTransaction(async (connection) => {
    ...
    const [questionRows] = await connection.query('SELECT * FROM assessment_questions WHERE assessment_id = ? ORDER BY id ASC', [assessmentId]);
    ...
    for (const question of questionRows) {
      ...
      if (isCorrect) score += Number(question.points || 1);
    }

    const answeredCount = Object.values(normalizedAnswers).filter(Boolean).length;
    if (!answeredCount) throw new Error('No answers were received by the server. Please answer the questions and submit again.');
    ...
    const answersPayload = JSON.stringify(normalizedAnswers);

    if (existing) {
      await connection.query(
        `UPDATE assessment_results
         SET score = ?, total_questions = ?, percentage = ?, level = ?, answers_json = ?, taken_at = SYSDATETIME()
         WHERE assessment_id = ? AND student_id = ?`,
        [score, questionRows.length, Number(percentage.toFixed(2)), level, answersPayload, assessmentId, studentId]
      );
    } else {
      await connection.query(
        `INSERT INTO assessment_results (assessment_id, student_id, score, total_questions, percentage, level, answers_json, taken_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, SYSDATETIME())`,
        [assessmentId, studentId, score, questionRows.length, Number(percentage.toFixed(2)), level, answersPayload]
      );
    }

    return { score, total_questions: questionRows.length, percentage, level, answered_count: answeredCount };
  });
}
  ...
  return withTransaction(async (connection) => {
    ...
    await connection.query(
      `UPDATE tutor_schedule_applications
       SET status = 'cancelled', updated_at = SYSDATETIME(), decided_at = SYSDATETIME(), decided_by = ?, student_notified = 0
    ...
    await connection.query(
      `UPDATE tutor_schedule_applications
       SET status = 'accepted', updated_at = SYSDATETIME(), decided_at = SYSDATETIME(), decided_by = ?, student_notified = 0
       WHERE id = ?`,
      [tutorId, app.id]
    );
    ...
    await connection.query(
      `UPDATE user_subject_assignments
       SET tutor_id = ?, time_slot = ?, assigned_at = SYSDATETIME(), accepted_by = ?, updated_at = SYSDATETIME()
...
async function markStudentScheduleNotificationRead(applicationId, studentId) {
  await query(
    `UPDATE tutor_schedule_applications SET student_notified = 1, updated_at = SYSDATETIME() WHERE id = ? AND student_id = ?`,
...
async function finishTutorScheduleApplication(applicationId, tutorId) {
  return withTransaction(async (connection) => {
    ...
    await connection.query(
      `UPDATE tutor_schedule_applications
       SET status = 'finished', updated_at = SYSDATETIME(), decided_at = SYSDATETIME(), decided_by = ?, student_notified = 0
       WHERE id = ?`,
      [tutorId, app.id]
    );
...
async function archiveSubjectResource(resourceId) {
  await query(
    `UPDATE subject_resources SET is_archived = 1, archived_at = SYSDATETIME(), updated_at = SYSDATETIME() WHERE id = ?`,
    [resourceId]
  );
}
...
async function recoverSubjectResource(resourceId) {
  await query(
    `UPDATE subject_resources SET is_archived = 0, recovered_at = SYSDATETIME(), updated_at = SYSDATETIME() WHERE id = ?`,
    [resourceId]
  );
}
...
async function advanceLearningCycle(cycleId, updates = {}) {
  const setClauses = ['updated_at = SYSDATETIME()'];
  const params = [];
...
async function completeOnlinePayment(paymentId) {
  ...
  await query(
    `UPDATE online_payments SET status = 'completed', paid_at = SYSDATETIME(), updated_at = SYSDATETIME() WHERE id = ?`,
    [paymentId]
  );
  ...
  if (bill) {
    ...
    await query(
      `UPDATE billing SET partial_payment = ?, for_settlement = ?, payment_status = ?, updated_at = SYSDATETIME() WHERE student_id = ?`,
    ...
    await query(
      `INSERT INTO payment_history (billing_id, student_id, amount, paid_at, recorded_by, transaction_type, payment_status, balance_after, remarks, payment_method, provider_reference)
       VALUES (?, ?, ?, SYSDATETIME(), NULL, 'online_payment', ?, ?, ?, 'online', ?)`,
...
async function respondToAssessmentRequest(requestId, tutorId, action, message = '') {
  ...
  await query(
    `UPDATE assessment_requests SET status = ?, responded_at = SYSDATETIME(), tutor_message = ?, updated_at = SYSDATETIME()
     WHERE id = ?`,
...
async function createPayMongoPayment(studentId, amount, billingInfo = {}) {
  ...
  if (!bill) throw new Error('No billing record found.');
  if (amount <= 0) throw new Error('Payment amount must be greater than zero.');
  if (amount < 100) throw new Error('Minimum online payment is ₱100.');
  const forSettlement = Number(bill.for_settlement || 0);
  if (amount > forSettlement) throw new Error(`Payment amount cannot exceed the remaining balance of ₱${forSettlement.toFixed(2)}.`);
