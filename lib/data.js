  ...
  return withTransaction(async (connection) => {
    ...
    for (const question of questionRows) {
      ...
      if (questionType === 'multiple choice') {
        ...
        normalizedAnswer = String(submittedRaw || '').trim();
        essayQuestions.push({ qid, question, answer: normalizedAnswer });
    ...
    if (existing) {
      await connection.query(
        `UPDATE assessment_results
         SET score = ?, total_questions = ?, percentage = ?, level = ?, answers_json = ?, taken_at = DATEADD(hour, 8, GETUTCDATE())
  ...
  return withTransaction(async (connection) => {
    ...
    for (const studentId of targetIds) {
      ...
      for (const q of questions) {
        await connection.query(
          `INSERT INTO assessment_questions (assessment_id, question_text, choice_a, choice_b, choice_c, choice_d, correct_answer, question_type, points, essay_rubric_keywords)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            assessmentId,
            q.question_text,
