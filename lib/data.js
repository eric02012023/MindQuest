...
async function getUsersPaged(options = {}) {
  ...
  const pageSize = Math.min(200, Math.max(5, Number(options.pageSize) || 25));
  ...
  return { rows: rows.map(parseRowArrays), total, page, pageSize, pageCount };
}
...
async function getBillingRows(scopeBranchId = null, onlyPaid = false, options = {}) {
  ...
  return query(
    `SELECT b.*, u.id AS student_id, u.user_id, u.first_name, u.middle_name, u.last_name,
...
async function completeOnlinePayment(paymentId) {
  ...
  await addPaymentEntry({
    ...
    purpose: 'Tuition',
    referenceNo: payment.provider_reference || null,
 ...
 */
async function getSubjectPostReadiness(subjectId) {
  const students = await query(
  ...
  const rows = [];
  for (const student of students) {
    const completion = await getStudentSubjectCompletion(student.id, subjectId);
    rows.push({ ...student, ...completion });
  }
  return {
    students: rows,
...
async function getSubmissionsByAssessment(assessmentId) {
  return query(
    `SELECT DISTINCT
     ...
     FROM tutor_assessments ta
     JOIN user_subject_assignments usa
          ON usa.subject_id = ta.subject_id AND usa.is_archived = 0
