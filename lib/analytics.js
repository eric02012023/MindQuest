...
async function getLevelSpread(scope, filters = {}) {
  ...
  let extra = '';
  if (filters.subjectId && String(filters.subjectId) !== 'all') {
    extra += ' AND ssl.subject_id = ?';
    params.push(Number(filters.subjectId));
  }
  if (scope.isTutor) {
    extra += ` AND EXISTS (
      SELECT 1 FROM user_subject_assignments usa4
      WHERE usa4.subject_id = ssl.subject_id AND usa4.tutor_id = ? AND usa4.is_archived = 0
    )`;
    params.push(scope.userId);
  }

  const rows = await query(
    `SELECT ssl.level, COUNT(*) AS total
     FROM student_subject_levels ssl
     JOIN users u ON u.id = ssl.student_id
     WHERE u.role = 'student' AND u.is_archived = 0${student.sql}${extra}
     GROUP BY ssl.level`,
    params
  );
  const spread = { Beginner: 0, Intermediate: 0, Advance: 0, Advanced: 0 };
  for (const row of rows) spread[row.level] = Number(row.total || 0);
  // 'Advanced' and 'Advance' both exist in the data; report them as one.
  spread.Advance += spread.Advanced;
  delete spread.Advanced;
  return spread;
}
...
async function getEnrolmentBreakdown(scope) {
  ...
  return query(
    `SELECT s.id AS subject_id, s.name AS subject_name,
            ...
            COUNT(DISTINCT usa.tutor_id) AS tutors
     FROM user_subject_assignments usa
     JOIN subjects s ON s.id = usa.subject_id
     JOIN users u ON u.id = usa.student_id
