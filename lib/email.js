...
function buildOtpHtml({ title, intro, otp, purpose }) {
  ...
  const purposeColor = purpose === 'login' ? '#4f46e5' : purpose === 'reset' ? '#dc2626' : '#7c3aed';
  const purposeLabel = purpose === 'login' ? 'Login Verification' : purpose === 'reset' ? 'Password Reset' : 'Email Verification';
...
async function sendOtpEmail({ to, otp, purpose = 'verification' }) {
  ...
  if (resendKey && resendKey.trim() !== '') {
    try {
      const from = process.env.RESEND_FROM || 'MindQuest <onboarding@resend.dev>';
      ...
      if (!response.ok) {
        ...
        console.warn(`Resend API Error: ${response.status} ${response.statusText} - ${errorText}. Falling back to SMTP...`);
      } else {
        return; // Success with Resend
      }
    } catch (e) {
      console.warn(`Resend fetch failed: ${e.message}. Falling back to SMTP...`);
    }
  }
