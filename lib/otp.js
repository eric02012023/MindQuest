...
async function storeOtp({ userId, purpose, code, expiresInMinutes = 10 }) {
  ...
  await query('UPDATE otps SET is_used = 1, updated_at = SYSDATETIME() WHERE user_id = ? AND purpose = ? AND is_used = 0', [Number(userId), purpose]);
  await query(
...
async function verifyOtp({ userId, purpose, code }) {
  ...
  if (row.expires_at && new Date(row.expires_at) < now) {
    await query('UPDATE otps SET is_used = 1, updated_at = SYSDATETIME() WHERE id = ?', [row.id]);
  ...
  await query('UPDATE otps SET is_used = 1, used_at = SYSDATETIME(), updated_at = SYSDATETIME() WHERE id = ?', [row.id]);
  return { ok: true, row };
}
...
async function getTrustedDevice(userId, token, fingerprint) {
  ...
  if (expiresAt && expiresAt < now) {
    await query('UPDATE trusted_devices SET is_active = 0, updated_at = SYSDATETIME() WHERE id = ?', [device.id]);
    return null;
  }
  return device;
}
...
async function deactivateTrustedDevice(userId, token) {
  ...
  await query('UPDATE trusted_devices SET is_active = 0, updated_at = SYSDATETIME() WHERE user_id = ? AND device_token = ?', [userId, token]);
}
...
async function touchTrustedDevice(id) {
  if (!id) return;
  const expiresAt = new Date(Date.now() + TRUSTED_DEVICE_DAYS * 24 * 60 * 60 * 1000);
  await query('UPDATE trusted_devices SET last_used_at = SYSDATETIME(), expires_at = ?, updated_at = SYSDATETIME() WHERE id = ?', [expiresAt, id]);
}
